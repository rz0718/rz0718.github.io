...
* workflow: systems where LLMs and tools are orchestrated through predefined code paths. Five major patterns are reviewed and visualized as below:
...
Agents leverage mature LLM capabilities to handle complex tasks through an interactive process. They start with user input, then independently plan and execute while gathering environmental feedback at each step. Human oversight remains possible through checkpoints and stopping conditions. Despite their sophistication, agents are fundamentally LLMs using tools in a feedback loop, making clear tool design and documentation essential for their effectiveness.
...
The agent operates in three key stages:
...
This planning process helps the agent stay organized and ensures all necessary steps are accounted for before execution begins.
